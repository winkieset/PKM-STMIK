...
class M_perpus extends CI_Model
{
	...
	{
		$sql = "insert into detail_pinjam (id_pinjam,id_pengajuan,denda) select transaksi.id_pinjam,transaksi.id_pengajuan,
    transaksi.denda from transaksi where transaksi.id_mahasiswa='$where'";
		$this->db->query($sql);
	}
	...
	public function find($where, $table)
	{
		...
		$hasil = $this->db->where('id_pengajuan', $where)
			->limit(1)
			->get($table);
		if ($hasil->num_rows() > 0) {
			return $hasil->row();
